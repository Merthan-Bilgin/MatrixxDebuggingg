	...
	public static void main(String[] args) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[0].length; j++) {
                transposedMatrix[i][j] = matrix[j][i]; // Corregido.
            }
        }
        ...
        try {
            System.out.println("\nAccediendo al elemento en [2][0]: " + matrix[2][0]); //Corregido
        ...
        try {
            ...
            for (int i = 0; i < matrix.length; i++) {  // Corregido
                for (int j = 0; j < matrix[i].length; j++) {  // Corregido
                    sum += matrix[i][j];
                }
            }
